...
function isAfghanistanIP(ip){
  ...
  // quick hint gate (same logic, tighter coverage)
  var maybe =
    ip.indexOf("58.147.")===0 ||
    ip.indexOf("59.153.")===0 ||
    ip.indexOf("61.5.")===0   ||
  ...
  // 103.* Afghanistan (expanded but tight)
  ...
  if (isInNet(ip, "103.106.0.0",  "255.255.0.0"))   return true; // AF block

  // 45.* Afghanistan
  if (isInNet(ip, "45.65.56.0",   "255.255.252.0")) return true;
  if (isInNet(ip, "45.116.128.0", "255.255.254.0")) return true;
  if (isInNet(ip, "45.116.0.0",   "255.255.0.0"))   return true;
...
function isJordanIP(ip){
  ...
  // CIDR confirmers (Jordan ISPs)
  if (isInNet(ip, "176.28.0.0",   "255.255.0.0"))   return true; // /16
  ...
  if (isInNet(ip, "185.37.0.0",   "255.255.0.0"))   return true; // /16
  if (isInNet(ip, "185.181.0.0",  "255.255.0.0"))   return true; // /16
...
function isGulfIP(ip){
  ...
  if (isInNet(ip, "185.195.0.0",   "255.255.0.0"))   return true;
  if (isInNet(ip, "185.196.0.0",   "255.255.0.0"))   return true;
  if (isInNet(ip, "94.26.0.0",     "255.255.0.0"))   return true;
  if (isInNet(ip, "95.177.0.0",    "255.255.0.0"))   return true;

  // UAE
  if (isInNet(ip, "5.62.0.0",      "255.255.0.0"))   return true;
  ...
  if (isInNet(ip, "217.164.0.0",   "255.255.0.0"))   return true;
  ...
  // New Gulf ranges (from table)
  if (isInNet(ip, "185.206.0.0",   "255.255.0.0"))   return true;
  if (isInNet(ip, "185.207.0.0",   "255.255.0.0"))   return true;
  if (isInNet(ip, "109.177.0.0",   "255.255.0.0"))   return true;
