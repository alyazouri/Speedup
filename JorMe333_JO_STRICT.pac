...
// =======================
// JORDAN CIDR (AS-based catch-all)
// Sources include AS9079 prefixes and Zain/Orange originated blocks (sampled)
// Update/extend as needed.
// =======================
var JO_CIDR = [
  // Orange / Jordan Telecom (AS8697) common aggregates
  ["212.34.0.0",   "255.255.224.0"], // /19 
  ["213.139.32.0", "255.255.224.0"], // /19 

  // Umniah / FirstNet (AS9079) originated
  ["185.80.106.0", "255.255.255.0"], // /24 
  ["185.80.107.0", "255.255.255.0"], // /24 

  // Zain Jordan (AS42912) aggregates (sample)
  ["37.123.64.0",  "255.255.224.0"], // /19 
  ["93.191.176.0", "255.255.248.0"], // /21 
  ["95.141.208.0", "255.255.240.0"], // /20 

  // Zain Data / Mobile (AS48832) aggregates (sample)
...
function isPUBG(host){
  ...
  // Tight but future-proof list (avoid 'te...' wildcards)
  return /(pubg|pubgm|pubgmobile|intlgame|igamecj|proximabeta|tencent|tencentyun|gcloud|qcloud|gameloop|vmp|kraken|battle|match|lobby|worldofwonder|ugc|creative|creation)/.test(host);
}
...
function getRealIPv4(host){
  ...
  if (DNS_CACHE[host] !== undefined) return DNS_CACHE[host];
  var ip = dnsResolve(host);
  DNS_CACHE[host] = (isIPv4(ip) ? ip : null);
...
function isJordanIP(ip){
  ...
  // CIDR catch-all (AS-based)
  for (var i=0;i<JO_CIDR.length;i++){
    if (isInNet(ip, JO_CIDR[i][0], JO_CIDR[i][1])) return true;
  }
...
var MATCH_JO_ONLY_MS = 120000; // 120s: Match JO-only, then JO/Gulf (optional)
var MATCH_START_TS = Date.now();
function matchPhase(){
  var dt = Date.now() - MATCH_START_TS;
  if (dt < MATCH_JO_ONLY_MS) return "JO_ONLY";
  return "JO_OR_GULF";
}
...
function FindProxyForURL(url, host){
  ...
  if (isMatchTraffic(url, host)) {
    ...
    // fallback: still keep it only JO or Gulf (never Europe/Asia)
    if (JO || GF) return MATCH_PROXY;
    return BLOCK;
  }
